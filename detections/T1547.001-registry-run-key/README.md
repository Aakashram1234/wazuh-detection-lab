...
The Threat Hunting dashboard showed a clear alert spike at the time of execution, with six level-12-or-above alerts in the last 24 hours:

![Threat Hunting dashboard with the alert spike](evidence/01-dashboard-overview.jpeg)

Filtering for `rule.id:100200` returned no results — the custom rule never matched, even though an alert had clearly fired on the test event:

![v1 rule produced no hits](evidence/02-rule-100200-empty-v1.jpeg)
...
![Wazuh built-in rule 92057 details, already tagged with MITRE T1059.001](evidence/03-built-in-92057-fired.jpeg)
...
![Rule 100200 firing at level 14 alongside built-in rule 92057](evidence/04-rule-100200-fired-v2.jpeg)

The decoded payload in both tests is harmless (`Write-Host "Hello from encoded powershell!"`). The detection fires on the *pattern*, not the payload. The earlier flag-free test at `18:56:28` correctly fired *only* 92057, not 100200 — confirming selective escalation works as designed.
